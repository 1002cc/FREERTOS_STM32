...
{ 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_11;	//指定6、11号引脚
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;	//指定6、7号引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//配置为输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//速度越高，功耗就越高，但是响应速度也更快
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;//不需要使能内部上下拉电阻
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	//指定8号引脚
	...
	//SCLK引脚配置为低电平，因为SPI工作模式0
	PDout(7)=0;
	
	//初始电平状态为高电平，因为总线空闲的时候，SS引脚（片选引脚）为高电平
	PDout(6)=1;
																			//启动SP3
}
//
